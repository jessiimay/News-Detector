...
var date_format = "$('#new_wrap').eq(0).attr(\"data\-publishtime\")";//
...
function seedget() {
    request(seedURL, function (err, res, body) { //读取种子页面
        try {
            // 用iconv转换编码
            var html = myIconv.decode(String(body), myEncoding);
            // console.log(html);
            //准备用cheerio解析html
        ...
        seedurl_news.each(function (i, e) { //遍历种子页面里所有的a链接
            ...
            var fetch_url_Sql = 'select url from chinanews where url=?';
...
function newsGet(myURL) { //读取新闻页面
    request(myURL, function (err, res, body) { //读取新闻页面
        ...
        var fetchAddSql = 'INSERT INTO chinanews(url,source_name,source_encoding,title,' +
